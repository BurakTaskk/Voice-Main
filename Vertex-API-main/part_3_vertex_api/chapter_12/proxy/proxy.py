...
# wav2lip-main klasörünü sys.path'e ekle (import'lardan önce)
import sys
WAV2LIP_PATH = pathlib.Path(__file__).resolve().parent.parent.parent.parent / "wav2lip-main"
if str(WAV2LIP_PATH) not in sys.path:
    sys.path.insert(0, str(WAV2LIP_PATH))

# Şimdi wav2lip modüllerini import et
import audio
...
CHUNK_SEC = 1  # WAV dosyası 0.3 sn'lik parçalara bölünür (1 saniyeden düşürüldü)
...
FACE_VIDEO_PATH = BASE_DIR / "bekleme9.mp4"
...
class WSManager:
    ...
    async def broadcast_video(self, payload: bytes):
        for ws in self.clients.copy():
            try:
                # Hedef adresi logla (host:port veya client objesi)
                client_info = getattr(ws, 'client', None)
                if client_info and hasattr(client_info, 'host') and hasattr(client_info, 'port'):
                    port = client_info.port if client_info.port != 0 else 8080
                    client_str = f"{client_info.host}:{port}"
                else:
                    client_str = str(client_info) if client_info else 'unknown'
                logging.info(f"Wav2Lip: Video segmenti gönderiliyor - {len(payload)} byte, client: {client_str}")
...
class Wav2LipService:
    ...
    def _mels(self, wav_path: str, fps: float):
        """
        wav dosyasını yükler, mel spektrogramını üretir ve
        fps'e göre 16-frame'lik pencerelere böler.
        """
        # 1) WAV'ı oku
        wav = audio.load_wav(wav_path, 16000)
        # 2) Mel spektrogramı üret
        mel = audio.melspectrogram(wav)
        # 3) fps'e göre adım büyüklüğünü hesapla
        step = 80.0 / fps
        mels = []
        i = 0
        # 4) 16 frame'lik bloklar halinde parçala
        while True:
            s = int(i * step)
            if s + 16 > mel.shape[1]:
                break
            mels.append(mel[:, s : s + 16])
            i += 1
        return mels

    def _pack_batch(self, ib, mb, fb, cb):
        """Batch verilerini tensor'lara dönüştürür."""
        ib, mb = np.asarray(ib), np.asarray(mb)
        mask = ib.copy(); mask[:, self.img_size//2:] = 0
        # → Sonuç shape: (B, H, W, 6)
        ib = np.concatenate((mask, ib), axis=3) / 255.0
        mb = np.expand_dims(mb, 3)  # (B, 80, 16, 1)

        t_i = torch.from_numpy(ib.transpose(0, 3, 1, 2)).float().to(self.device)
        t_m = torch.from_numpy(mb.transpose(0, 3, 1, 2)).float().to(self.device)
        return t_i, t_m, fb, cb

    def _datagen(self, frames, mels, boxes):
        """Batch generator for processing frames and mels."""
        ib, mb, fb, cb = [], [], [], []
        single = len(frames) == 1
        if single:
        ...
        for i, m in enumerate(mels):
            ...
            ib.append(face); mb.append(m); fb.append(frames[idx].copy()); cb.append(boxes[idx])
            if len(ib) >= self.batch:
                yield self._pack_batch(ib, mb, fb, cb)
                ib, mb, fb, cb = [], [], [], []
        if ib:
            yield self._pack_batch(ib, mb, fb, cb)

    def _blend(self, frame: np.ndarray, face_patch: np.ndarray, coord: Tuple[int,int,int,int]) -> np.ndarray:
        """
        face_patch: model çıktısı (örnek boyut 96×96×3)
        coord     : (x1, y1, x2, y2) bölge koordinatları
        """
        x1, y1, x2, y2 = coord
        w, h = x2 - x1, y2 - y1
        # eğer bölge yoksa orijinali döndür
        if w <= 0 or h <= 0:
            return frame

        # face_patch'i ROI boyutuna yeniden ölçekle
        try:
            face_resized = cv2.resize(face_patch, (w, h), interpolation=cv2.INTER_LINEAR)
        except Exception as e:
            # Ölçekleme başarısızsa orijinal frame'i döndür
            print(f"[WARN] _blend resize failed: {e}")
            return frame
    ...
    def make_mp4(self, wav_b64: str, video_start_frame: int = 0) -> Tuple[bytes, int]:
        """
        base64 WAV → dudak senkronlu MP4 baytları.
        Belirtilen başlangıç karesinden itibaren referans video karelerini kullanır.
        Dönüş: (video_bytes, işlenen_kare_sayısı)
        """
        ...
        num_audio_frames = len(mels) # Sesin kaç video karesine denk geldiğini hesapla
        if not mels:
            shutil.rmtree(tmp_dir, ignore_errors=True)
            return b"", 0 # Eğer ses yoksa, boş bayt ve 0 kare döndür

        # --- YENİ KARE SEÇME MANTIĞI ---
        total_video_frames = len(self.frames)
        
        # video_start_frame'den başlayarak num_audio_frames kadar kare al.
        # Video sonuna gelinirse, baştan devam et (loop).
        indices = [(video_start_frame + i) % total_video_frames for i in range(num_audio_frames)]
        
        frames = [self.frames[i] for i in indices]
        boxes = [self.boxes_cache[i] for i in indices]
        # --- YENİ KARE SEÇME MANTIĞI SONU ---
        ...
        # --- _datagen ile batch işle ---
        for t_img, t_mel, f_batch, c_batch in self._datagen(frames, mels, boxes):
        ...
        shutil.rmtree(tmp_dir, ignore_errors=True)
        # Fonksiyonun dönüşünü, işlenen kare sayısıyla birlikte yap
        return data, num_audio_frames
...
# fastapi_app tanımı lifespan fonksiyonunda yapılacak
...
async def worker():
    total_frames_processed = 0  # Toplam işlenen kare sayısını tutan sayaç
    segment_count = 0
    while True:
        ...
        try:
            ...
            # Bu ses segmenti için video başlangıç noktasını belirle
            current_video_frame_offset = total_frames_processed

            for seg in stream_wav_chunks(base64.b64decode(wav_b64)):
                total_sub_segments += 1
                seg_b64 = base64.b64encode(seg).decode()
                
                # Değiştirilmiş make_mp4'ü çağır, mevcut video ofsetini parametre olarak ver
                mp4, frames_in_this_chunk = await asyncio.to_thread(
                    svc.make_mp4, seg_b64, video_start_frame=current_video_frame_offset
                )
                
                if mp4:
                    logging.info(f"Wav2Lip: Segment #{segment_count}, sub-segment {total_sub_segments} gönderiliyor, boyut: {len(mp4)} byte")
                    await mgr.broadcast_video(mp4)
                    logging.info(f"   ↳ Segment #{segment_count}, {total_sub_segments}. sub-segment yollandı.")
                    
                    # Bir sonraki ses parçası için video ofsetini güncelle
                    current_video_frame_offset += frames_in_this_chunk

            # Tüm segment işlendiğinde, genel sayacı güncelle
            total_frames_processed = current_video_frame_offset

        except Exception:
            logging.exception(f"Segment #{segment_count} işleme hatası")
        finally:
            await mgr.broadcast_done(total_sub_segments)
            logging.info(f"⏹  Segment #{segment_count} tamamlandı ({total_sub_segments} sub-segment). Toplam işlenen kare: {total_frames_processed}")
...
async def command_ws(ws: WebSocket):
    ...
    try:
        while True:
            ...
            logging.info("Wav2Lip: Ses dosyası kuyruğa alındı (WAV/PCM) [WebSocket]")
            await queue.put(b64)
...
async def upload_wav(request: FastAPIRequest):
    ...
    logging.info("Wav2Lip: Ses dosyası kuyruğa alındı (WAV/PCM) [upload_wav]")
    await queue.put(data["audio_b64"])
    return {"status": "queued", "segment": segment_name}

@fastapi_app.post("/wav2lip/upload_pcm")
async def upload_pcm(request: FastAPIRequest):
    data = await request.json()
    ...
    logging.info("Wav2Lip: Ses dosyası kuyruğa alındı (WAV/PCM) [upload_pcm]")
...
async def send_audio_to_wav2lip(base64_audio, format_type="audio/pcm"):
    ...
    import json
    try:
        uri = "ws://localhost:8000/wav2lip/ws/command"  # wav2lip sunucusunun adresi
                ...
                if "setup" in data:
                    ...
                    if turn_complete:
                        ...
                        if all_audio:
                            ...
                            try:
                                # PCM'yi WAV'a dönüştür ve Wav2Lip kuyruğuna ekle
                                wav_b64 = pcm_to_wav_b64(base64.b64encode(all_audio).decode())
                                await queue.put(wav_b64)
                                print("[Proxy] Ses Wav2Lip kuyruğuna eklendi")
                                
                                # Dosyayı da kaydet (debug için)
...
async def weather_handler(request: FastAPIRequest):
...
async def main() -> None:
    ...
    # FastAPI app'i aiohttp'ya mount et
    # 1) Köprüyü oluştur - kök altına mount ediyoruz (varsayılan root_path="/")
    asgi_res = ASGIResource(fastapi_app)
    
    # 2) Aiohttp router'ına *resource* olarak ekle
    app.router.register_resource(asgi_res)
    
    # 3) FastAPI'nin lifespan'ını aiohttp döngüsüne bağla
    asgi_res.lifespan_mount(app)
